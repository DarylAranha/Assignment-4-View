...
class APICRUDViewController: UIViewController, UITableViewDelegate, UITableViewDataSource
{
    ...
    func fetchMovies(completion: @escaping ([Movie]?, Error?) -> Void)
    {
        ...
        guard let url = URL(string: "https://mdev1004-2023-assignment-4.onrender.com/api/list") else
        {
            print("URL Error")
            ...
            do {
                ...
                if let success = json?["success"] as? Bool, success == true
                {
                    if let moviesData = json?["data"] as? [[String: Any]] {
                        do {
                            let jsonData = try JSONSerialization.data(withJSONObject: moviesData, options: [])
    ...
    func deleteMovie(at indexPath: IndexPath)
    {
        ...
        guard let url = URL(string: "https://mdev1004-2023-assignment-4.onrender.com/api/delete/\(movie._id)") else {
            print("Invalid URL")
            return
        }
